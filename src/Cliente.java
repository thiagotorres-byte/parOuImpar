...
class Cliente2 implements Runnable {
    ...
    public void start() throws IOException {
        try {
            clienteObject2 = new ClienteObject(
                    new Socket(SERVER_ADRESS, Servidor.PORT)
            );
            ...
            clienteObject2.close();
        }
    }

    private void enviarMensagemLoopVsCPU() {

        clienteObject2.sendMessage(msg);
    ...
    public void run() {
        String msg;
        try {
            while ((msg = clienteObject2.getMessage()) != null) {
                if ("sair".equalsIgnoreCase(msg))
                    return;
                System.out.println(msg);
            }
